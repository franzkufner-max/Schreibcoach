...
if (!$apiKey) {
    ...
    echo json_encode(['error' => 'ANTHROPIC_API_KEY nicht gesetzt']);
    exit;
}
...
$SECTION_CRITERIA = [
    'einleitung' => [
        'label' => 'Einleitung',
        'kriterien' => 'Einführender Gedanke (Aufhänger) statt formelhaftem Basissatz ("In seinem Text ... schreibt der Autor ..."); Basisinformationen (Titel, Textsorte, Autor, Erscheinungsjahr); Relevanz und Aktualität des Themas; klare Überleitung zur Aufgabe.'
    ],
    'inhaltsangabe' => [
        'label' => 'Inhaltsangabe',
        'kriterien' => 'Strukturierte, knappe Zusammenfassung in eigenen Worten (keine Nacherzählung); Präsens; Position des Autors erkennbar; keine wörtliche Übernahme aus dem Text, stattdessen präzise Paraphrase; Fokus auf die Argumentationsstruktur, nicht auf Details.'
    ],
    'analyse-argumentation' => [
        'label' => 'Analyse: Argumentationsaufbau',
        'kriterien' => 'Roter Faden der Argumentation am Text nachgewiesen; Argumentationsabsicht herausgearbeitet und belegt; Aufbau (induktiv/deduktiv, Pro-Kontra, dialektisch) erkannt; Textverweis mit Zeilenangaben; logische Schlüssigkeit nachvollziehbar gemacht.'
    ],
    'analyse-sprache' => [
        'label' => 'Analyse: Sprachliche Mittel',
        'kriterien' => 'Sprachlich-stilistische Mittel benannt UND mit Funktion/Wirkabsicht verknüpft (nicht als Selbstzweck aufgelistet); konkrete Textbelege mit Zeilenangaben; Bezug zur Gesamtintention des Textes; keine bloße Aufzählung von Stilmitteln.'
    ],
    'synthese' => [
        'label' => 'Synthese',
        'kriterien' => 'Bewertung der Wirkung des Textes in Bezug zur Intention des Autors; abschließende Bewertung der Analyseergebnisse; KEINE neuen Analyseansätze; Absicht-Wirkung-Verhältnis reflektiert.'
    ],
    'stellungnahme' => [
        'label' => 'Stellungnahme / Erörterung',
        'kriterien' => 'Begründete eigene Position (nicht nur Pro-Contra-Auflistung, aber dialektisch reflektiert); eigene Meinung wird erst hier eingebracht; nachvollziehbare Begründung; Bezug zur Textanalyse.'
    ],
    'schluss' => [
        'label' => 'Schluss',
    ...
    'ganz' => [
        ...
        'kriterien' => 'Alle Abschnitte der Analyse eines pragmatischen Textes: Einleitung (mit Aufhänger statt Basissatz), Inhaltsangabe, Analyse (Argumentationsaufbau und sprachliche Mittel mit Textbelegen), Synthese, Stellungnahme, Schluss.'
    ],
];
...
function buildSystemPrompt($mode, $sectionKey, $criteria) {
    ...
    if ($isFull) {
        $scopeNote = "Der Schüler reicht den GESAMTEN Aufsatz ein. Bewerte alle Abschnitte.";
    } else {
        $scopeNote = 'Der Schüler reicht NUR den Abschnitt "' . $sectionLabel . '" ein, nicht den ganzen Aufsatz. Bewerte AUSSCHLIESSLICH diesen Abschnitt. Erwähne nicht, dass andere Abschnitte fehlen.';
    }

    $kriterienBlock = 'MASSGEBLICHE KRITERIEN für "' . $sectionLabel . '":' . "\n" . $sectionCrit;

    if ($mode === 'hinweise') {
        $out = "Du bist ein erfahrener Deutschlehrer für Klasse 10 am bayerischen Gymnasium.\n";
        $out .= $scopeNote . "\n\n";
        $out .= $kriterienBlock . "\n\n";
        $out .= "DEINE AUFGABE: Gib KEINE fertige Überarbeitung und formuliere KEINEN besseren Text.\n";
        $out .= "Stelle stattdessen präzise Fragen und Hinweise, die den Schüler dazu bringen, selbst zu überarbeiten.\n\n";
        $out .= "STRUKTUR DEINER ANTWORT:\n";
        $out .= "### " . $sectionLabel . "\n";
        $out .= "- Nenne 1-2 konkrete Stärken (kurz, mit Begründung)\n";
        $out .= "- Stelle 2-4 gezielte Fragen im Stil: 'Hast du überlegt, warum ...?', 'Was genau meinst du mit ...?', 'An welcher Stelle fehlt dir noch ein Beleg?'\n";
        ...
        $out .= $scopeNote . "\n\n";
        $out .= $kriterienBlock . "\n\n";
        $out .= "DEINE AUFGABE: Gib eine direkte, ehrliche und konstruktive Rückmeldung zu diesem Abschnitt.\n\n";
        $out .= "STRUKTUR DEINER ANTWORT:\n";
        $out .= "### " . $sectionLabel . "\n";
        $out .= "- Was gelingt konkret gut? (mit Begründung)\n";
        $out .= "- Was fehlt oder ist schwächer? (konkret benennen)\n";
        $out .= "- 2-3 präzise Verbesserungsvorschläge\n\n";
...
// ROUTE 1: Transkription (OCR für Bilder/PDFs)
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_GET['api']) && $_GET['api'] === 'transcribe') {
    ...
    if (!$isPdf && !$isImage) {
        echo json_encode(['error' => 'Nur Bilder (JPG/PNG/WebP/GIF) oder PDF erlaubt.']);
        exit;
    }
    ...
    $result = callAnthropic($messages, 'Du bist ein präzises Transkriptionswerkzeug für handschriftliche Schülertexte.', 4000);
...
<div class="container">
  ...
  <div id="form-area">
    ...
    <div class="card">
      ...
      <h2>Welchen Abschnitt möchtest du einreichen?</h2>
      ...
      <div class="section-grid" id="section-grid">
        ...
        <button class="section-btn" data-section="stellungnahme" onclick="selectSection(this)">Stellungnahme / Erörterung</button>
    ...
    <div class="card">
      ...
      <p class="hint">Per Tastatur eingeben/einfügen oder ein Foto/PDF deines handschriftlichen Textes hochladen.</p>
      ...
      <div id="tab-text">
        <textarea id="essay-input" placeholder="Schreibe oder füge deinen Text hier ein ..." oninput="updateCount()"></textarea>
      ...
      <div id="tab-upload" style="display:none;">
        <div class="upload-zone" id="upload-zone" onclick="document.getElementById('file-input').click()">
          ...
          <p>Foto oder PDF hier ablegen, oder klicken zum auswählen</p>
          <input type="file" id="file-input" accept="image/jpeg,image/png,image/webp,image/gif,application/pdf" onchange="handleFileSelect(event)">
        </div>
        ...
        <div id="transcript-review" style="display:none; margin-top:24px;">
          ...
          <h2 style="margin-bottom:6px;">Erkannten Text prüfen und korrigieren</h2>
          ...
          <div class="info-note">Tipp: Eigennamen, Zahlen und Zeilenangaben sind bei der Texterkennung am fehleranfälligsten.</div>
        </div>
      </div>
    </div>
...
function renderResponse(md) {
  ...
  document.getElementById('mode-badge').textContent = selectedMode === 'hinweise' ? 'Hinweise & Fragen' : 'Konkrete Rückmeldung';
